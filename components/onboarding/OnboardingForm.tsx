...
const INITIAL_STEPS = ['Boas-vindas', 'Workspace', 'Canais', 'Foto', 'Revisar'];
const WORKSPACE_STEPS = ['Workspace', 'Canais', 'Revisar'];
const WORKSPACE_SCREEN_STEPS = [2, 3, 5] as const;
const DRAFT_VERSION = 3;
type FormData = { workspaceName: string; firstName: string; lastName: string; professionalProfile: string; referralSource: string; selectedChannels: OnboardingChannel[]; brandName: string; logoUrl: string; photoUrl: string; instagramHandle: string; newsInstagramHandle: string; twitterHandle: string; palette: string[]; niche: string; audience: string; brandStory: string; audiencePains: string; defaultTone: string };
type Draft = Partial<FormData> & { step?: number; version?: number };
const empty = (): FormData => ({ workspaceName: '', firstName: '', lastName: '', professionalProfile: '', referralSource: '', selectedChannels: ['instagram_carousel'], brandName: '', logoUrl: '', photoUrl: '', instagramHandle: '', newsInstagramHandle: '', twitterHandle: '', palette: COLORS, niche: '', audience: '', brandStory: '', audiencePains: '', defaultTone: '' });
const validStep = (step: unknown, count = INITIAL_STEPS.length) => typeof step === 'number' && step >= 1 && step <= count ? step : 1;
const draftStep = (draft: Draft) => {
  const step = validStep(draft.step, INITIAL_STEPS.length);
  // Version 2 already used the five-step initial wizard. Keep its step
  // numbers intact while version 3 adds the workspace-only draft shape.
  if (draft.version === 2 || draft.version === DRAFT_VERSION) return step;
  return step === 2 ? 3 : step === 3 ? 4 : step === 4 ? 5 : step;
};
const workspaceDraftStep = (draft: Draft) => {
  const step = validStep(draft.step, INITIAL_STEPS.length);
  if (draft.version === DRAFT_VERSION) return validStep(step, WORKSPACE_STEPS.length);
  return step === 5 ? 3 : step === 3 || step === 4 ? 2 : 1;
};
...
export default function OnboardingForm({ onComplete, compact = false }: { onComplete?: () => void; compact?: boolean }) {
  ...
  const [initialOnboarding, setInitialOnboarding] = useState(true);
  ...
  const steps = initialOnboarding ? INITIAL_STEPS : WORKSPACE_STEPS;
  const stepCount = steps.length;
  const screenStep = initialOnboarding ? step : WORKSPACE_SCREEN_STEPS[step - 1] ?? 2;
  ...
  useEffect(() => {
    ...
    const load = async (id: string) => {
      activeUserId = id; setLoading(true); setUserId(id); setWorkspaceId(''); setData(empty()); setStep(1); setInitialOnboarding(true); setPhotoFile(null); setLogoPreviewUrl('');
      ...
      if (draft && alive) {
        ...
        setLogoPreviewUrl(typeof draft.logoUrl === 'string' ? draft.logoUrl : '');
      ...
      const isInitialOnboarding = !profile?.onboarding_completed;
      setInitialOnboarding(isInitialOnboarding);
      if (draft) setStep(isInitialOnboarding ? draftStep(draft) : workspaceDraftStep(draft));
      if (profile && !draft) {
        ...
        setLogoPreviewUrl(value('logo_url', ''));
      }
      setLoading(false);
    };
  ...
  const save = async (complete: boolean, silent = false, throwOnError = false) => {
    ...
    if (complete && (!data.brandName.trim() || !data.selectedChannels.length || missingChannel)) {
      const message = !data.brandName.trim() ? 'Informe o nome da marca.' : 'Informe o @ de cada canal selecionado.';
      if (throwOnError) throw new Error(message);
      if (!silent) toast.error(message);
      return false;
    }
    const payload = initialOnboarding
      ? data
      : {
        ...
        newsInstagramHandle: data.newsInstagramHandle,
    ...
    const response = await fetch('/api/onboarding', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...payload, complete }) });
    if (!response.ok) {
      const message = (await response.json().catch(() => ({}))).error || 'Não foi possível salvar.';
      if (throwOnError) throw new Error(message);
      if (!silent) toast.error(message);
      return false;
    }
  ...
  const move = (nextStep: number) => { const safe = Math.min(stepCount, Math.max(1, nextStep)); setStep(safe); persistDraft(data, safe); };
  const submit = async (event: React.FormEvent) => {
    event.preventDefault();
    if (step !== stepCount) { move(step + 1); return; }
    setSaving(true);
    const completion = save(true, true, true);
    try {
      await toastManager.promise(completion, {
        ...
        error: (error) => ({ title: 'Não foi possível concluir onboarding', description: error instanceof Error ? error.message : 'Tente novamente.' }),
      });
      onComplete?.();
    } catch {
      // O toast exibe o erro; o onboarding continua aberto para correção.
    } finally {
      setSaving(false);
    }
  };
  ...
  const chooseLogo = async (file: File) => {
    ...
    try {
      const uploadedUrl = await uploadImageFile(file, 'workspace-logos');
      update('logoUrl', uploadedUrl);
      setLogoPreviewUrl(uploadedUrl);
      toast.success('Logo pronta para salvar.');
    } catch (error) {
