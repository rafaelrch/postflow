...
vi.mock('@/components/onboarding/GlobalOnboardingModal', () => ({
  default: () => <div data-testid="onboarding-modal">setup bloqueante</div>,
}));
...
describe('AuthProvider — gate de onboarding_completed', () => {
  it('mantém uma camada bloqueante enquanto verifica o perfil autenticado', async () => {
    mockGetSession.mockReturnValue(new Promise(() => undefined));

    const screen = await renderAuthProvider();

    expect(screen.getByTestId('onboarding-loading')).toBeTruthy();
    expect(screen.queryByText('conteúdo protegido')).toBeTruthy();
  });

  it('abre o popup global bloqueante, sem tirar o usuário da tela atual, quando o onboarding está pendente', async () => {
  ...
  it('abre o popup quando o profile ainda não existe (linha de corrida do trigger)', async () => {
    mockSingle.mockResolvedValue({ data: null });

    await renderAuthProvider();

    await waitFor(() => expect(document.querySelector('[data-testid="onboarding-modal"]')).toBeTruthy());
    expect(mockReplace).not.toHaveBeenCalled();
  });

  it('abre o popup bloqueante para conta pendente mesmo quando já se está em /onboarding', async () => {
